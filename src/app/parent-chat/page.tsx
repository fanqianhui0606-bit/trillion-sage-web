"use client";

import { useState, useEffect, useRef } from "react";
import Link from "next/link";
import Image from "next/image";
import html2canvas from "html2canvas";
import { jsPDF } from "jspdf";

interface Message {
  sender: "ai" | "user";
  text: string;
  agentName?: string;
  borderColor?: string;
}

interface ReportData {
  decision_role: string;
  soft_moment: string;
  parenting_style: string;
  warm_suggestion: string;
}

interface StudentReportData {
  mirror_echo: string;
  math_aesthetic: string;
  reality_guide: string;
}

interface FamilyLinkStatus {
  student_completed?: boolean;
  parent_completed?: boolean;
  student_authorized?: boolean;
  parent_authorized?: boolean;
  student_report?: StudentReportData;
  parent_report?: ReportData;
  paid?: boolean;
}

const OPENING_QUESTIONS = [
  "您好。这几年带孩子，一定很辛苦吧？这里是一个不需要您伪装坚强的树洞。有什么烦心事，或者关于孩子的困惑，都可以跟我聊聊。",
  "家长您好，恭喜孩子终于考完了。比起孩子的成绩，我们其实更关心您的感受。这些年操心这么多，有没有觉得自己也有点累了？",
  "您好。很多家长在孩子长大后，会突然觉得不知道该怎么和他们沟通了。这里没有别人，您可以安心地放下包袱，跟我聊聊最近的想法。"
];

function cleanJsonString(str: string): string {
  let cleaned = str.trim();
  if (cleaned.startsWith("```")) {
    cleaned = cleaned.replace(/^```(json)?/, "").replace(/```$/, "").trim();
  }
  return cleaned;
}

export default function ParentChatPage() {
  const [messages, setMessages] = useState<Message[]>([]);
  const [inputValue, setInputValue] = useState("");
  const [isTyping, setIsTyping] = useState(false);
  const [turn, setTurn] = useState(1);
  const [reportData, setReportData] = useState<ReportData | null>(null);
  const messagesEndRef = useRef<HTMLDivElement | null>(null);
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const reportRef = useRef<HTMLDivElement | null>(null);
  const [isDownloading, setIsDownloading] = useState(false);
  const [isPageReady, setIsPageReady] = useState(false);

  // Family Link States
  const [familyCode, setFamilyCode] = useState<string | null>(null);
  const [isAuthorized, setIsAuthorized] = useState(false);
  const [linkStatus, setLinkStatus] = useState<FamilyLinkStatus | null>(null);
  const [studentReport, setStudentReport] = useState<StudentReportData | null>(null);
  const [checkingStatus, setCheckingStatus] = useState(false);
  const [activationInput, setActivationInput] = useState("");
  const [unlockError, setUnlockError] = useState("");

  const handleUnlockReport = async () => {
    if (!activationInput.trim()) {
      setUnlockError("请输入激活码");
      return;
    }
    setUnlockError("");
    try {
      const res = await fetch("/api/family-link", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          code: familyCode,
          role: "parent",
          action: "pay",
          activationCode: activationInput.trim().toUpperCase()
        })
      });
      if (res.ok) {
        const data = await res.json();
        setLinkStatus(data);
        if (data.student_report) {
          setStudentReport(data.student_report);
        }
      } else {
        const data = await res.json();
        setUnlockError(data.error || "激活失败，请检查密钥是否正确");
      }
    } catch (err) {
      console.error(err);
      setUnlockError("网络连接错误，请稍后再试");
    }
  };

  // Initialize opening message & load family code
  useEffect(() => {
    // Load code
    const code = sessionStorage.getItem("family_code");
    if (code) {
      setFamilyCode(code);
      // Fetch initial link status if code exists
      fetch(`/api/family-link?code=${code}&role=parent`)
        .then((r) => r.json())
        .then((data) => {
          setLinkStatus(data);
          if (data.parent_authorized) {
            setIsAuthorized(true);
          }
          if (data.parent_completed && data.parent_report) {
            setReportData(data.parent_report);
          }
        })
        .catch(console.error);
    }

    const timer = setTimeout(() => {
      const randomQuestion = OPENING_QUESTIONS[Math.floor(Math.random() * OPENING_QUESTIONS.length)];
      setMessages([
        {
          sender: "ai",
          text: randomQuestion,
          agentName: "主理人",
          borderColor: "border-bridge-blue/20 bg-white/80 text-stone-850 shadow-sm",
        },
      ]);
      setTurn(1);
      // If we already loaded reportData, don't clear it
      setIsPageReady(true);
    }, 1500);
    return () => clearTimeout(timer);
  }, []);

  // Scroll to bottom on new messages
  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [messages, isTyping, reportData]);

  // Background candle flickering & star dust animation (Warmer colors for parents)
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;

    const ctx = canvas.getContext("2d");
    if (!ctx) return;

    let animId: number;
    let w = (canvas.width = window.innerWidth);
    let h = (canvas.height = window.innerHeight);

    const handleResize = () => {
      if (!canvas) return;
      w = canvas.width = window.innerWidth;
      h = canvas.height = window.innerHeight;
    };
    window.addEventListener("resize", handleResize);

    const dust: { x: number; y: number; r: number; alpha: number; speed: number }[] = [];
    for (let i = 0; i < 40; i++) {
      dust.push({
        x: Math.random() * w,
        y: Math.random() * h,
        r: Math.random() * 1.5 + 0.5,
        alpha: Math.random() * 0.4 + 0.1,
        speed: 0.003 + Math.random() * 0.005,
      });
    }

    let flicker = 1.0;

    const render = () => {
      ctx.clearRect(0, 0, w, h);

      flicker += (Math.random() - 0.5) * 0.08;
      if (flicker > 1.25) flicker = 1.25;
      if (flicker < 0.8) flicker = 0.8;

      const grad = ctx.createRadialGradient(
        w / 2, h * 0.95, 20,
        w / 2, h * 0.85, 300 * flicker
      );
      // Warmer, earthy tones for the parent interface
      grad.addColorStop(0, "rgba(180, 83, 9, 0.08)"); 
      grad.addColorStop(0.4, "rgba(217, 119, 6, 0.03)");
      grad.addColorStop(1, "rgba(0, 0, 0, 0)");

      ctx.fillStyle = grad;
      ctx.fillRect(0, 0, w, h);

      for (const d of dust) {
        d.alpha += d.speed;
        if (d.alpha > 0.65 || d.alpha < 0.05) d.speed *= -1;
        ctx.fillStyle = `rgba(217, 119, 6, ${Math.max(0, d.alpha)})`;
        ctx.beginPath();
        ctx.arc(d.x, d.y, d.r, 0, Math.PI * 2);
        ctx.fill();
      }

      animId = requestAnimationFrame(render);
    };

    render();

    return () => {
      cancelAnimationFrame(animId);
      window.removeEventListener("resize", handleResize);
    };
  }, []);

  const checkLinkStatus = async (codeStr: string) => {
    setCheckingStatus(true);
    try {
      const res = await fetch(`/api/family-link?code=${codeStr}&role=parent`);
      if (res.ok) {
        const data = await res.json();
        setLinkStatus(data);
        if (data.student_authorized && data.parent_authorized) {
          setStudentReport(data.student_report);
        }
      }
    } catch (err) {
      console.error(err);
    } finally {
      setCheckingStatus(false);
    }
  };

  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!inputValue.trim() || isTyping) return;

    const userText = inputValue;
    const newMessages = [...messages, { sender: "user" as const, text: userText }];
    setMessages(newMessages);
    setInputValue("");
    setIsTyping(true);

    const nextTurn = turn + 1;
    
    // Inject system directive at 10th round for parents
    const payloadMessages = [...newMessages];
    if (nextTurn === 10) {
      payloadMessages.push({
        sender: "user",
        text: "[SYSTEM_DIRECTIVE]: REPORT"
      });
    }

    try {
      const res = await fetch("/api/parent-chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ messages: payloadMessages, turn: nextTurn })
      });

      if (!res.ok) throw new Error("API Error");

      const data = await res.json();
      
      if (nextTurn === 10) {
        let parsedReport = null;
        try {
          const cleanedText = cleanJsonString(data.text);
          parsedReport = JSON.parse(cleanedText);
          setReportData(parsedReport);
        } catch (err) {
          console.error("Failed to parse report JSON:", err);
          parsedReport = {
            decision_role: "从交流中能感觉到，您为家庭决策承担了主要的重量，这是一份深沉的责任感。",
            soft_moment: "您提到孩子默默用功或理解您的情景，那是无论时光怎么流逝，都在您心里最柔软的角落。",
            parenting_style: "面对分歧，您虽然偶尔会急躁，但出发点总是为了孩子好。您是一位理性且充满爱的家长。",
            warm_suggestion: "了解孩子是一个需要终身学习的课题。如果您觉得最近和他沟通有些吃力，不必有挫败感，这只是因为他进入了新的阶段。如果您愿意，随时可以联系我们的导师团队。"
          };
          setReportData(parsedReport);
        }

        // 1. Submit Chat Log for Submissions ("留档")
        try {
          await fetch("/api/chat-submissions", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              role: "parent",
              code: familyCode || "体验版",
              messages: payloadMessages,
              report: parsedReport
            })
          });
        } catch (subErr) {
          console.error("Failed to archive chat log:", subErr);
        }

        // 2. Submit for Family Linking if code exists
        if (familyCode) {
          try {
            await fetch("/api/family-link", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                code: familyCode,
                role: "parent",
                report: parsedReport,
                action: "submit"
              })
            });
            checkLinkStatus(familyCode);
          } catch (linkErr) {
            console.error("Failed to submit family link:", linkErr);
          }
        }
      } else {
        const agentName = "主理人";
        const borderStyle = "border-bridge-blue/20 bg-white/80 text-stone-850 shadow-sm";

        setMessages((prev) => [
          ...prev,
          {
            sender: "ai",
            text: data.text,
            agentName,
            borderColor: borderStyle,
          },
        ]);
      }
      setTurn(nextTurn);
    } catch (err) {
      console.error(err);
      setMessages((prev) => [
        ...prev,
        {
          sender: "ai",
          text: "（网络出现了一点小波动，请稍后再试...）",
          agentName: "系统提示",
          borderColor: "border-red-500/30",
        },
      ]);
    } finally {
      setIsTyping(false);
    }
  };

  const handleDownloadPDF = async () => {
    if (!reportRef.current || isDownloading) return;
    setIsDownloading(true);

    try {
      const canvas = await html2canvas(reportRef.current, {
        scale: 2,
        useCORS: true,
        backgroundColor: "#2a1f1a" // Warm dark background
      });

      const imgData = canvas.toDataURL("image/jpeg", 1.0);
      const pdf = new jsPDF("p", "mm", "a4");
      const pdfWidth = pdf.internal.pageSize.getWidth();
      const pdfHeight = (canvas.height * pdfWidth) / canvas.width;
      
      pdf.addImage(imgData, "JPEG", 0, 0, pdfWidth, pdfHeight);
      pdf.save("家长专属简报.pdf");
    } catch (error) {
      console.error("Failed to generate PDF:", error);
      alert("生成 PDF 失败，请稍后再试。");
    } finally {
      setIsDownloading(false);
    }
  };

  return (
    <div className="h-screen flex flex-col pt-16 relative overflow-hidden">
      {/* Background Canvas */}
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" />

      {/* Chat area — fixed layout */}
      <div className="flex-1 flex flex-col max-w-4xl w-full mx-auto px-4 md:px-6 relative z-10 overflow-hidden">
        {/* Header */}
        <div className="px-6 py-3 border-b-2 border-bridge-blue/30 flex items-center justify-between bg-white/60 backdrop-blur-md shrink-0 rounded-t-xl">
            <div className="flex items-center gap-3">
              <div className="w-3 h-3 rounded-full bg-bridge-blue animate-pulse" />
              <h1 className="font-serif text-bridge-blue-dark tracking-widest text-sm md:text-base font-bold">
                家长倾听室 {familyCode && <span className="text-xs bg-bridge-blue/10 px-2 py-0.5 rounded font-mono text-bridge-blue ml-2">关联码: {familyCode}</span>}
              </h1>
              <span className="text-xs text-bridge-muted/60 font-mono hidden md:inline">
                对话轮数: {turn}/10
              </span>
            </div>
            <Link
              href="/"
              className="text-indigo-700 hover:text-indigo-900 font-serif text-xs px-3 py-1 border-2 border-indigo-300 rounded-md bg-indigo-50 hover:bg-indigo-100 font-semibold transition-all"
            >
              返回官网首页
            </Link>
          </div>

          {/* Messages — scrollable */}
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6 bg-white/40">
            {!reportData && messages.map((msg, idx) => (
              <div
                key={idx}
                className={`flex flex-col max-w-[85%] md:max-w-[75%] ${
                  msg.sender === "user" ? "ml-auto items-end" : "mr-auto items-start animate-slide-up"
                }`}
              >
                {msg.sender === "ai" && msg.agentName && (
                  <span className="text-[10px] md:text-xs font-serif text-bridge-blue-dark/80 mb-1.5 ml-2 tracking-wider font-semibold">
                    {msg.agentName}
                  </span>
                )}
                <div
                  className={`p-4 rounded-lg text-sm md:text-base leading-relaxed tracking-wide ${
                    msg.sender === "user"
                      ? "bg-bridge-blue/10 text-stone-900 border border-bridge-blue/20 rounded-tr-none shadow-sm"
                      : `bg-white text-stone-850 border border-bridge-blue/10 rounded-tl-none shadow-sm ${msg.borderColor || ""}`
                  }`}
                >
                  {msg.text}
                </div>
              </div>
            ))}

            {(!isPageReady || isTyping) && (
              <div className="mr-auto items-start flex flex-col max-w-[70%] animate-pulse">
                <span className="text-xs font-serif text-bridge-blue/50 mb-1.5 ml-2 tracking-wider">
                  倾听中...
                </span>
                <div className="bg-white/60 p-4 border border-stone-200 rounded-lg rounded-tl-none flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 bg-bridge-blue-light rounded-full animate-bounce" style={{ animationDelay: "0ms" }} />
                  <span className="w-1.5 h-1.5 bg-bridge-blue-light rounded-full animate-bounce" style={{ animationDelay: "150ms" }} />
                  <span className="w-1.5 h-1.5 bg-bridge-blue-light rounded-full animate-bounce" style={{ animationDelay: "300ms" }} />
                </div>
              </div>
            )}

            {/* Step A: Authorization Panel (when code exists but parent not authorized) */}
            {reportData && familyCode && !isAuthorized && (
              <div className="animate-fade-in w-full max-w-xl mx-auto py-12 px-6 bg-white/80 border border-bridge-blue/20 rounded-xl shadow-md flex flex-col items-center text-center gap-6">
                <div className="w-12 h-12 bg-bridge-blue/10 rounded-full flex items-center justify-center border border-bridge-blue/30 text-bridge-blue text-xl font-bold">
                  🔮
                </div>
                <div>
                  <h3 className="font-serif text-lg font-bold text-stone-850 tracking-wider">
                    对话已顺利收敛，请授权查看报告
                  </h3>
                  <p className="text-xs text-bridge-muted leading-relaxed mt-2 font-serif max-w-sm">
                    您的这端倾听已顺利完成。为了对比分析您与孩子的思维默契度，生成双端共振报告，我们需要您的授权同意。
                  </p>
                </div>

                <div className="p-3 bg-stone-50 border border-stone-200 rounded font-mono text-xs text-stone-700">
                  您的家庭关联码为: <span className="font-bold text-bridge-blue select-all">{familyCode}</span>
                </div>

                <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                  <button
                    onClick={async () => {
                      try {
                        await fetch("/api/family-link", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({
                            code: familyCode,
                            role: "parent",
                            action: "authorize"
                          })
                        });
                        setIsAuthorized(true);
                        checkLinkStatus(familyCode);
                      } catch (err) {
                        console.error(err);
                      }
                    }}
                    className="px-6 py-2.5 bg-bridge-blue hover:bg-bridge-blue-dark text-white rounded font-serif text-sm tracking-widest shadow-sm hover:shadow-md transition-all flex-1"
                  >
                    同意授权并对比
                  </button>
                  <button
                    onClick={async () => {
                      try {
                        await fetch("/api/family-link", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({
                            code: familyCode,
                            role: "parent",
                            action: "deny"
                          })
                        });
                        setFamilyCode(null); // Bypass family linking, view single report
                      } catch (err) {
                        console.error(err);
                        setFamilyCode(null);
                      }
                    }}
                    className="px-6 py-2.5 border-2 border-indigo-300 hover:border-indigo-400 text-indigo-700 hover:text-indigo-900 rounded font-serif text-sm tracking-widest transition-all flex-1 font-semibold"
                  >
                    暂不授权
                  </button>
                </div>
              </div>
            )}

            {/* Step B: Waiting Panel (when parent authorized, but student has not) */}
            {reportData && familyCode && isAuthorized && linkStatus && (!linkStatus.student_completed || !linkStatus.student_authorized) && (
              linkStatus.student_authorized === false ? (
                <div className="animate-fade-in w-full max-w-xl mx-auto py-12 px-6 bg-white/80 border border-bridge-blue/20 rounded-xl shadow-md flex flex-col items-center text-center gap-6">
                  <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center border border-red-200 text-red-500 text-xl font-bold">
                    🔒
                  </div>
                  <div>
                    <h3 className="font-serif text-lg font-bold text-stone-850 tracking-wider">
                      少年端已选择暂不授权
                    </h3>
                    <p className="text-xs text-bridge-muted leading-relaxed mt-2 font-serif max-w-sm">
                      由于少年端选择了暂不公开报告，出于对其直觉隐私和独立成长空间的保护，系统已为您阻断双端对比。
                    </p>
                  </div>
                  <button
                    onClick={() => setFamilyCode(null)}
                    className="px-6 py-2.5 bg-bridge-blue hover:bg-bridge-blue-dark text-white rounded font-serif text-sm tracking-widest shadow-sm hover:shadow-md transition-all font-semibold"
                  >
                    查看我自己的单端简报
                  </button>
                </div>
              ) : (
                <div className="animate-fade-in w-full max-w-xl mx-auto py-12 px-6 bg-white/80 border border-bridge-blue/20 rounded-xl shadow-md flex flex-col items-center text-center gap-6">
                  <div className="w-12 h-12 border-2 border-bridge-blue border-t-transparent rounded-full animate-spin" />
                  
                  <div>
                    <h3 className="font-serif text-lg font-bold text-stone-850 tracking-wider">
                      正在等待学生端接入并授权...
                    </h3>
                    <p className="text-xs text-bridge-muted leading-relaxed mt-2 font-serif max-w-sm">
                      您已授权。请把您的家庭关联码发送给孩子，让他们在首页选择“学生端”进入并输入此码进行聊天。
                    </p>
                  </div>

                  <div className="p-3 bg-stone-50 border border-stone-200 rounded font-mono text-xs text-stone-700">
                    家庭关联码: <span className="font-bold text-bridge-blue select-all">{familyCode}</span>
                  </div>

                  <div className="flex gap-4">
                    <button
                      onClick={() => checkLinkStatus(familyCode)}
                      disabled={checkingStatus}
                      className="px-6 py-2.5 bg-bridge-blue hover:bg-bridge-blue-dark text-white rounded font-serif text-sm tracking-widest shadow-sm hover:shadow-md transition-all disabled:opacity-50"
                    >
                      {checkingStatus ? "刷新中..." : "刷新状态"}
                    </button>
                    <button
                      onClick={() => {
                        // Bypass waiting and display single report
                        setFamilyCode(null);
                      }}
                      className="px-6 py-2.5 border-2 border-indigo-300 hover:border-indigo-400 text-indigo-700 rounded font-serif text-sm tracking-widest transition-all bg-white font-semibold"
                    >
                      先看我自己的单端简报
                    </button>
                  </div>
                </div>
              )
            )}

            {/* Step C: Display final report */}
            {reportData && (!familyCode || (isAuthorized && studentReport)) && (
              <div className="animate-fade-in w-full flex flex-col items-center pb-12">
                <div 
                  ref={reportRef} 
                  className="w-full max-w-2xl bg-gradient-to-b from-[#FAF7F2] to-[#F3EDE2] p-8 md:p-12 rounded-xl shadow-[0_10px_35px_rgba(46,117,182,0.08)] border border-bridge-blue/20 relative"
                >
                  <div className="absolute top-0 left-0 w-16 h-16 border-t border-l border-bridge-blue/40 rounded-tl-xl" />
                  <div className="absolute top-0 right-0 w-16 h-16 border-t border-r border-bridge-blue/40 rounded-tr-xl" />
                  <div className="absolute bottom-0 left-0 w-16 h-16 border-b border-l border-bridge-blue/40 rounded-bl-xl" />
                  <div className="absolute bottom-0 right-0 w-16 h-16 border-b border-r border-bridge-blue/40 rounded-br-xl" />
                  
                  <div className="text-center mb-10">
                    <h2 className="text-xl md:text-2xl font-serif text-bridge-blue-dark tracking-[0.2em] mb-2 font-bold">
                      ☕ {studentReport ? "家庭思维相干" : "岁月沉淀"}
                    </h2>
                    <p className="text-xs text-bridge-blue/60 tracking-widest font-serif">
                      {studentReport ? "学生与家长双端思维共振报告" : "专属家长简报"}
                    </p>
                  </div>

                  <div className="space-y-10">
                    {/* Student Section (if comparison mode enabled) */}
                    {studentReport && (
                      <div className="space-y-6">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="w-1.5 h-4 bg-bridge-blue rounded" />
                          <h3 className="font-serif text-base font-bold text-bridge-blue-dark">少年端 · 认知探索</h3>
                        </div>
                        
                        <div>
                          <h4 className="text-sm font-serif text-bridge-blue mb-2.5 border-b border-bridge-blue/15 pb-2 font-semibold">
                            一、 镜像回响：我们看见的你
                          </h4>
                          <p className="text-stone-700 text-sm leading-relaxed font-serif text-justify whitespace-pre-wrap">
                            {studentReport.mirror_echo}
                          </p>
                        </div>

                        <div>
                          <h4 className="text-sm font-serif text-bridge-blue mb-2.5 border-b border-bridge-blue/15 pb-2 font-semibold">
                            二、 数理审美：你的直觉纹理
                          </h4>
                          <p className="text-stone-700 text-sm leading-relaxed font-serif text-justify whitespace-pre-wrap">
                            {studentReport.math_aesthetic}
                          </p>
                        </div>

                        <div>
                          <h4 className="text-sm font-serif text-bridge-blue mb-2.5 border-b border-bridge-blue/15 pb-2 font-semibold">
                            三、 破壁指南：给你的现实线索
                          </h4>
                          <p className="text-stone-700 text-sm leading-relaxed font-serif text-justify whitespace-pre-wrap">
                            {studentReport.reality_guide}
                          </p>
                        </div>
                      </div>
                    )}

                    {/* Parent Section */}
                    <div className={`space-y-6 ${studentReport ? "border-t border-stone-300/60 pt-8" : ""}`}>
                      {studentReport && (
                        <div className="flex items-center gap-2 mb-2">
                          <span className="w-1.5 h-4 bg-amber-600 rounded" />
                          <h3 className="font-serif text-base font-bold text-amber-900">家长端 · 默默托举</h3>
                        </div>
                      )}

                      {/* Section 1 */}
                      <div>
                        <h3 className="text-sm font-serif text-amber-700 mb-3 border-b border-amber-900/15 pb-2 font-semibold flex items-center gap-2">
                          <span className="text-amber-600/60">◆</span> 家庭决策画像
                        </h3>
                        <p className="text-stone-700 text-sm leading-relaxed font-serif text-justify whitespace-pre-wrap">
                          {reportData.decision_role}
                        </p>
                      </div>

                      {/* Section 2 */}
                      <div>
                        <h3 className="text-sm font-serif text-amber-700 mb-3 border-b border-amber-900/15 pb-2 font-semibold flex items-center gap-2">
                          <span className="text-amber-600/60">◆</span> 最柔软的瞬间
                        </h3>
                        <p className="text-stone-700 text-sm leading-relaxed font-serif text-justify whitespace-pre-wrap">
                          {reportData.soft_moment}
                        </p>
                      </div>

                      {/* Section 3 */}
                      <div>
                        <h3 className="text-sm font-serif text-amber-700 mb-3 border-b border-amber-900/15 pb-2 font-semibold flex items-center gap-2">
                          <span className="text-amber-600/60">◆</span> 沟通模式镜像
                        </h3>
                        <p className="text-stone-700 text-sm leading-relaxed font-serif text-justify whitespace-pre-wrap">
                          {reportData.parenting_style}
                        </p>
                      </div>

                      {/* Section 4 */}
                      <div className="bg-amber-600/5 p-5 rounded-lg border border-amber-600/10 mt-6">
                        <h3 className="text-xs font-serif text-amber-800 mb-3 flex items-center gap-2 font-bold">
                          <span className="text-base">💌</span> 给家长的一封信
                        </h3>
                        <p className="text-stone-750 text-xs leading-relaxed font-serif italic whitespace-pre-wrap">
                          {reportData.warm_suggestion}
                        </p>
                      </div>
                    </div>

                    {/* Double Consent Analysis Block */}
                    {studentReport && (
                      <div className="bg-bridge-blue/5 p-6 rounded-lg border border-bridge-blue/15 mt-8 space-y-3">
                        <h4 className="text-xs font-serif text-bridge-blue-dark mb-1 font-bold flex items-center gap-1.5">
                          <span>🌟</span> 双端共振分析：量子纠缠图景 (免费摘要)
                        </h4>
                        <p className="text-stone-700 text-xs leading-loose font-serif text-justify">
                          在这个突然失去重力的暑假，少年的直觉在极力挣脱教条规则的重力束缚，寻找属于个体的自由退相干空间；而家长的视线里虽满含对未来不确定性的隐忧，却最终用爱达成了对边界探索的克制留白。
                          分歧之中，少年的“战略应付”并非叛逆，而是理智资源的自我保护；家长的“理智引导”也并非束缚，而是将担忧化为了默默的相干屏蔽。
                          你们之间的引力并不依靠言语黏合，而是在理解与留白的不变量中产生共振。少年的每一次无畏逃逸，都源于背后有一双温润的双手在充当稳定的阻尼器。请带着这层共振，勇敢走向现实的粗糙。
                        </p>
                      </div>
                    )}

                    {/* Paid Lock Overlay Component */}
                    {studentReport && linkStatus && !linkStatus.paid && (
                      <div className="mt-8 border border-bridge-gold/30 bg-amber-50/15 rounded-lg p-6 relative overflow-hidden text-center animate-fade-in">
                        <div className="absolute top-2 right-3 text-bridge-gold/60 text-[9px] font-mono tracking-widest animate-pulse">
                          PREVIEW 🔒
                        </div>
                        <h4 className="font-serif text-sm text-amber-900 font-bold mb-2">
                          《双端引力相干与心智对称性分析手册》已生成
                        </h4>
                        <p className="text-[11px] text-bridge-muted max-w-md mx-auto mb-5 leading-relaxed">
                          这是一份针对您家庭定制的高级深层次心智发展与升学阻力分析报告（包含四个高维度解析板块）。您可通过微信支付解锁。
                        </p>
                        
                        <div className="flex flex-col md:flex-row gap-5 items-center justify-center bg-white/70 border border-stone-200/60 p-4 rounded-lg">
                          <div className="w-16 h-16 relative flex-shrink-0 bg-white p-0.5 rounded border border-stone-200">
                            <Image
                              src="/images/wechat.jpg"
                              alt="微信公众号"
                              width={64}
                              height={64}
                              className="object-contain"
                            />
                          </div>
                          <div className="text-left flex-1 space-y-3">
                            <p className="text-[10px] text-stone-500 font-serif leading-relaxed">
                              <strong>如何获取解锁激活码？</strong><br />
                              微信扫码关注「**桥梁计划Bridge**」，发送您的家庭关联码 <span className="font-mono text-bridge-blue font-bold">{familyCode}</span>。付款解锁后，输入助理发给您的激活密钥（即 <code className="bg-amber-100 px-1 py-0.5 rounded font-mono text-[9px]">VIP-{familyCode}</code>）。
                            </p>
                            
                            <div className="flex gap-2 w-full">
                              <input
                                type="text"
                                value={activationInput}
                                onChange={(e) => {
                                  setActivationInput(e.target.value);
                                  setUnlockError("");
                                }}
                                placeholder="激活码例如: VIP-XXXXXX"
                                className="flex-1 px-3 py-1.5 bg-white border border-stone-300 rounded focus:border-bridge-gold/50 focus:outline-none text-[11px] font-mono"
                              />
                              <button
                                type="button"
                                onClick={handleUnlockReport}
                                className="px-4 py-1.5 bg-bridge-gold hover:bg-amber-600 text-white rounded font-serif text-[11px] tracking-wider transition-colors shadow-sm"
                              >
                                解锁报告
                              </button>
                            </div>
                            {unlockError && (
                              <p className="text-[10px] text-red-500 font-serif mt-1 font-semibold">{unlockError}</p>
                            )}
                          </div>
                        </div>
                      </div>
                    )}

                    {/* Paid Unlock Report Content */}
                    {studentReport && linkStatus && linkStatus.paid && (
                      <div className="space-y-8 border-t border-dashed border-bridge-gold/30 pt-8 mt-8 animate-fade-in text-left">
                        <div className="text-center mb-6">
                          <h3 className="font-serif text-sm font-bold text-amber-950 tracking-wider">
                            《双端引力相干与心智对称性分析手册》已解锁 🔓
                          </h3>
                        </div>

                        {/* 板块一 */}
                        <div className="bg-amber-500/5 p-4 border border-amber-500/10 rounded-lg">
                          <h4 className="text-xs font-serif text-amber-900 font-bold mb-2 border-b border-amber-900/10 pb-1.5 flex items-center gap-1.5">
                            <span>一、</span> 相干性检视：引力不变量
                          </h4>
                          <p className="text-stone-750 text-xs leading-loose font-serif text-justify">
                            从双方的思维轨迹投影来看，我们看见了两种不同维度的心智状态：少年的星轨直觉在力求脱离“定轨运动”，以求在最真实的自由退相干空间里探索自我兴趣；而家长则因为对外部理工专业复杂性的认知，在做着充满焦虑与爱意的“引力托举”。在这个相互作用力下，摩擦与抗拒是必然的物理表象。然而，这并非结构缺陷，恰恰是你们家庭系统在长线变轨中得以维持稳定飞行的“引力不变量”。这种引力由底层的爱和共同的理性求知本能支撑，它不因局部的分歧而消退，反而在经历退相干时成为最终能把双方重新黏合的引力不变量。
                          </p>
                        </div>

                        {/* 板块二 */}
                        <div className="bg-amber-500/5 p-4 border border-amber-500/10 rounded-lg">
                          <h4 className="text-xs font-serif text-amber-900 font-bold mb-2 border-b border-amber-900/10 pb-1.5 flex items-center gap-1.5">
                            <span>二、</span> 阻尼与阻抗：热力学第二定律的家庭投影
                          </h4>
                          <p className="text-stone-750 text-xs leading-loose font-serif text-justify">
                            如果将家庭沟通抽象为一个封闭的热力学孤立系统，我们可以清晰地观测到其内部的熵增过程。当外界关于升学压力、选科方向的噪声侵入时，系统会自发地通过情绪的湍流来耗散能量。少年的“战略应付”与阶段性沉默，并非抗拒沟通，而在物理本质上是其面对无效噪声时的一种“信息压缩与系统负熵本能”——他在用这层阻抗防止自己最昂贵的抽象注意力被消耗殆尽。而家长的焦灼介入，则是一场做功过程，若没有缓冲，势必带来局部的热量积聚与热阻升高。最佳的调适路径是引入“理性阻尼器”，即允许分歧以弹性的阻尼振荡形式自然衰减，而不是强求一朝一夕的绝对同频，从而达到系统的最优能量退相干。
                          </p>
                        </div>

                        {/* 板块三 */}
                        <div className="bg-amber-500/5 p-4 border border-amber-500/10 rounded-lg">
                          <h4 className="text-xs font-serif text-amber-900 font-bold mb-2 border-b border-amber-900/10 pb-1.5 flex items-center gap-1.5">
                            <span>三、</span> 量子相干与退相干：寻找自由退相干空间
                          </h4>
                          <p className="text-stone-750 text-xs leading-loose font-serif text-justify">
                            少年在成长期中的最大特权，是保留其数理直觉的多维相干叠加态——他既学着适应高考的刚性要求，又对高等物理、数学的纯粹思想拥有极高的好奇心。这种相干态极易因过早的工具化和应试对错判断而塌缩。家长所能提供的高级守护，不是随时扮演观测者去测量他，而是充当“相干屏蔽罩”，用充满留白和距离的温暖防线，为他提供一片不被打扰的自由退耦合空间。
                          </p>
                        </div>

                        {/* 板块四 */}
                        <div className="bg-amber-500/5 p-4 border border-amber-500/10 rounded-lg">
                          <h4 className="text-xs font-serif text-amber-900 font-bold mb-2 border-b border-amber-900/10 pb-1.5 flex items-center gap-1.5">
                            <span>四、</span> 共振导航：未来引力平衡路线图
                          </h4>
                          <div className="text-stone-750 text-[11px] leading-loose font-serif space-y-2">
                            <p>我们为您整理了如下落地的引力调适建议：</p>
                            <ul className="list-disc list-inside space-y-1.5 pl-1 text-[11px] text-stone-700">
                              <li><strong>以物理审美代替公式督导</strong>：当面临孩子日常学习的马虎时，尝试跳出试卷上的对错判断，去探讨其在解决难题时展现的独特逻辑，让防备的阻抗状态转化为同频的引力共振。</li>
                              <li><strong>在边界的边缘进行弱耦合</strong>：在大方向志愿选择上，以学术信息提供者的身份进行弱耦合（如分享清北前沿学科营资料），将变轨决定权留给少年本身。</li>
                              <li><strong>充当其现实粗糙 of 阻尼器</strong>：孩子具有极高灵性思考，但也容易有空中楼阁的失重感。请以您人生经验的温润与稳定，接纳他的挫败，陪他在现实的粗糙面上磨砺出真正的数理人格。</li>
                            </ul>
                          </div>
                    {/* 免责声明 */}
                    <div className="border-t border-stone-300/40 pt-4 mt-8 text-[9px] text-stone-400 font-serif leading-relaxed text-center">
                      本报告由 AI 语言模型基于对话及测验自动生成，内容仅作为亲子学业沟通与潜质发掘之参考，不构成任何心理诊断、临床治疗建议或升学/选科之专业决定依据。若面临心理危机，请寻求专业咨询或医疗机构帮助。
                    </div>
                  </div>
                </div>
                    )}

                {/* External Actions & Links */}
                <div className="w-full max-w-2xl mt-8 space-y-6">
                  <div className="flex justify-center">
                    <button
                      onClick={handleDownloadPDF}
                      disabled={isDownloading}
                      className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-serif text-sm rounded-md transition-all flex items-center gap-2 disabled:opacity-50 font-semibold tracking-wider"
                    >
                      {isDownloading ? "生成中..." : "⬇ 下载并保存当前简报"}
                    </button>
                  </div>

                  <div className="bg-white/80 p-6 rounded-xl border border-stone-200/60 shadow-sm text-left">
                    <p className="text-center text-bridge-blue-dark font-serif mb-6 tracking-widest text-sm font-semibold">
                      「 守护引航，给孩子更好的托举 」
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <Link href="/quiz" className="flex flex-col justify-between p-4 rounded-lg bg-stone-50 hover:bg-stone-100 border border-stone-200 hover:border-bridge-blue/30 transition-all group">
                        <div>
                          <span className="text-[10px] font-serif bg-bridge-blue/10 text-bridge-blue px-2 py-0.5 rounded font-semibold tracking-wider">素质定量化</span>
                          <p className="text-stone-700 font-serif text-xs leading-relaxed mt-2 group-hover:text-bridge-blue-dark transition-colors">想客观检测孩子的数理直觉？让孩子开启14维科学素质测评</p>
                        </div>
                        <span className="text-[10px] text-stone-400 group-hover:text-bridge-blue/60 mt-4">&rarr; 开始14维测验</span>
                      </Link>
                      <Link href="/#programs-preview" className="flex flex-col justify-between p-4 rounded-lg bg-stone-50 hover:bg-stone-100 border border-stone-200 hover:border-amber-600/30 transition-all group">
                        <div>
                          <span className="text-[10px] font-serif bg-amber-500/10 text-amber-700 px-2 py-0.5 rounded font-semibold tracking-wider">学科营探索</span>
                          <p className="text-stone-700 font-serif text-xs leading-relaxed mt-2 group-hover:text-amber-800 transition-colors">理工类专业怎么选？了解千殊九大前沿数理专业线上衔接营</p>
                        </div>
                        <span className="text-[10px] text-stone-400 group-hover:text-amber-700 mt-4">&rarr; 了解线上衔接营</span>
                      </Link>
                      <Link href="/#consultation" className="flex flex-col justify-between p-4 rounded-lg bg-bridge-gold/5 hover:bg-bridge-gold/10 border border-bridge-gold/20 transition-all group">
                        <div>
                          <span className="text-[10px] font-serif bg-bridge-gold/15 text-bridge-gold px-2 py-0.5 rounded font-semibold tracking-wider">清北导师1v1</span>
                          <p className="text-stone-700 font-serif text-xs leading-relaxed mt-2">破除大学科研与行业信息差？预约清华/北京大学硕博团队1v1引航</p>
                        </div>
                        <span className="text-[10px] text-bridge-gold mt-4">&rarr; 预约1v1引航</span>
                      </Link>
                    </div>
                  </div>
                </div>
                  </div>
                </div>
              </div>
            )}

            <div ref={messagesEndRef} />
          </div>

          {/* Input — fixed at bottom */}
          {!reportData && (
            <div className="flex flex-col border-t-2 border-bridge-blue/30 bg-white/60 backdrop-blur-md shrink-0 rounded-b-xl">
              <form onSubmit={handleSend} className="p-4 flex gap-3">
                <input
                  type="text"
                  value={inputValue}
                  onChange={(e) => setInputValue(e.target.value)}
                  disabled={isTyping}
                  placeholder={isTyping ? "等待回应..." : "想说点什么？"}
                  className="flex-1 px-4 py-2 bg-white border-2 border-indigo-200 rounded-md focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 focus:outline-none/50 focus:outline-none text-stone-800 text-sm tracking-wide transition-all disabled:opacity-50"
                />
                <button
                  type="submit"
                  disabled={isTyping || !inputValue.trim()}
                  className="px-6 py-2 bg-bridge-blue text-white font-serif text-sm tracking-widest rounded-md hover:bg-bridge-blue-dark disabled:opacity-50 disabled:hover:bg-bridge-blue transition-all duration-300 cursor-pointer"
                >
                  发送
                </button>
              </form>
              <p className="text-[10px] text-stone-400 font-serif text-center pb-2 px-4 select-none">
                本服务为 AI 辅助思维探索，非专业心理咨询或医学诊断。若面临情绪崩溃或心理危机，请寻求专业咨询或医疗热线帮助。
              </p>
            </div>
          )}
        </div>
      </div>
  );
}
